...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            // Deletion successful, redirect to the same page to refresh the list
...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    // Ensure page is at least 2 for AJAX calls
    $page = max(2, (int)($_GET['page'] ?? 2)); // Default to page 2 for the first AJAX call

    // Calculate offset and limit for this page
    // Page 1 is loaded by the initial PHP script (EVENTS_PER_INITIAL_LOAD events)
    // AJAX calls start from page 2 onwards.
    // The offset for page 2 is EVENTS_PER_INITIAL_LOAD.
    // The offset for page 3 is EVENTS_PER_INITIAL_LOAD + EVENTS_PER_LOAD_AJAX.
    // The offset for page N is EVENTS_PER_INITIAL_LOAD + (N - 2) * EVENTS_PER_LOAD_AJAX.
    $offset = EVENTS_PER_INITIAL_LOAD + ($page - 2) * EVENTS_PER_LOAD_AJAX;
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            document.querySelectorAll('.dashboard-btn-edit').forEach(button => { // Updated selector
                button.addEventListener('click', (event) => { // Pass the event object
                    ...
                    .then(data => {
                        ...
                        if (data.events.length > 0) {
                            ...
                            data.events.forEach(event => {
                                ...
                                // After adding new elements, re-attach event listeners to the new edit buttons
                                eventsGrid.querySelectorAll('.dashboard-btn-edit').forEach(editButton => {
                                    // Remove existing listener to avoid duplicates
                                    // This is a simplified approach; a better way might involve event delegation
                                    editButton.removeEventListener('click', handleEditClick);
                                    editButton.addEventListener('click', handleEditClick);
                                });
                                // Similarly, re-attach listeners for delete buttons if needed (though form submission might be sufficient)
                                // eventsGrid.querySelectorAll('.dashboard-btn-delete').forEach(deleteButton => { ... });
                            });
            ...
            // Define the edit click handler function to easily re-attach it
            ...
            // Attach initial event listeners to edit buttons
